...
def insertmenu():
    ...
    for line in file:
            menu = line.split(',')
            m = {'item':menu[0], 'store':int(menu[4]), 'cal':int(menu[2]), 'filter':menu[1], 'price':int(menu[3]), 'href':menu[5], 'src':menu[6]}
...
def showmenu():
    ...
    r['data'] = []
    for doc in docs:
            ret = {'item':doc['item'], 'store':doc['store'], 'cal':doc['cal'], 'filter':doc['filter'], 'price':doc['price'], 'pic':doc['href'], 'src':doc['src']}
            r['data'].append(ret)
    return jsonify(r)
